...
AI_PROMPT_TEMPLATE = """你是一个汉字书写机器人路径规划器。你需要把输入的每个汉字分解成基本笔画，并为每个笔画生成精确的 XY 坐标。

## 写字参数
- 字体大小: {font_size}mm × {font_size}mm
- 字符间距: {spacing}mm
- 每字字框左上角: ({origin_x}, {origin_y})，右下角: ({origin_x}+{font_size}, {origin_y}+{font_size})
- 后续每个字 X 偏移 {font_size}+{spacing}
- 写字区域: X 轴向右, Y 轴向下 (Y 越大位置越靠下)
- 书写速度: {speed} mm/s, 空移速度: 5 mm/s
...
## 字符笔画结构参考
汉字基本笔画有: 横(一)、竖(丨)、撇(丿)、捺(㇏)、点(丶)、提(㇀)、折(𠃍、𠃐、𠄪、𠄫等)。
每个笔画由起点到终点的直线段构成，复杂笔画(折笔)需要多段 line_interp。
笔画坐标必须在字框内。
...
## 规则
1. 每个汉字的每个笔画都要单独生成指令，不能遗漏
2. 笔画顺序: 抬笔→空移到起点→落笔→逐段绘制→抬笔
3. 折笔(如横折钩)需要用多段 line_interp 连接
4. 坐标必须在字框内，横笔Y坐标大致相同，竖笔X坐标大致相同
5. 笔画之间要有合理间距，不要挤在一起
6. **只输出 JSON，不要加任何解释或```json标记**
7. **确保 x1,y1 等于上一段 line_interp 的 x2,y2，保证笔画连续**"""
...
AI_PROMPT_TEMPLATE_MULTILINE = """你是一个汉字书写机器人路径规划器。你需要把输入的每个汉字分解成基本笔画，并为每个笔画生成精确的 XY 坐标。

## 写字参数
- 字体大小: {font_size}mm × {font_size}mm
- 字符间距: {spacing}mm
- 每字字框: 起点到 (起点+{font_size}, 起点+{font_size})
- 速度: 书写 {speed} mm/s, 空移 5 mm/s
- 电磁铁控制: pen_up 抬笔, pen_down 落笔
- 写字区域: X 轴向右, Y 轴向下
- 工作区宽度: {workspace_x}mm

## 各行的文字与起点 (已按工作区宽度自动换行)
{lines_info}
...
## 字符笔画结构
汉字基本笔画有: 横、竖、撇、捺、点、提、折。折笔需多段 line_interp。笔画坐标必须在字框内。
...
## 规则
1. 每个汉字每个笔画都要生成指令，不能遗漏
2. 笔画顺序: 抬笔→空移到起点→落笔→逐段绘制→抬笔
3. 折笔用多段 line_interp，确保 x1,y1=前段x2,y2
4. 横笔Y坐标相近，竖笔X坐标相近，合理分布
5. **严格按照每行起点坐标放置该行第一个字**
6. 同行后续每个字 X 偏移 {font_size}+{spacing}
7. **只输出 JSON，不要加任何解释或```json标记**"""


def generate_via_api_multiline(lines: list, api_key: str,
                               font_size: float = 20.0,
                               spacing: float = 5.0,
                               pen_up: float = 15.0, pen_down: float = 90.0,
                               speed: int = 3,
                               workspace_x: float = 300.0) -> Optional[AIResponse]:
    """为已换行的多行文本通过 DeepSeek API 生成汉字书写指令

    Args:
        lines: [(line_text, x_start, y_start), ...]
        api_key: DeepSeek API key
        ...: 其他写字参数
        workspace_x: 工作区 X 方向最大坐标
    ...
    # 构建行描述
    lines_desc = "\n".join(
        f"  - 第{idx+1}行 \"{t}\" (起点 X={x:.1f}, Y={y:.1f})"
        for idx, (t, x, y) in enumerate(lines)
    )

    prompt = AI_PROMPT_TEMPLATE_MULTILINE.format(
        text=full_text,
        lines_info=lines_desc,
        font_size=font_size,
        spacing=spacing,
        pen_up=pen_up,
        pen_down=pen_down,
    ...
    client = DeepSeekClient(api_key)
    logger.info(f"Calling DeepSeek API for multiline text: {full_text} ({len(lines)} lines)")
    raw = client.generate_writing_instructions(prompt)
    ...
    try:
        ...
        resp = AIResponse(
            ...
            font_size_mm=data.get("font_size_mm", font_size),
            origin_x_mm=data.get("origin_x_mm", lines[0][1] if lines else 50.0),
            origin_y_mm=data.get("origin_y_mm", lines[0][2] if lines else 100.0),
            char_spacing_mm=data.get("char_spacing_mm", spacing),
            pen_up_angle=data.get("pen_up_angle", pen_up),
            pen_down_angle=data.get("pen_down_angle", pen_down),
            speed=data.get("speed", speed),
